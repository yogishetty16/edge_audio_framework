...
def load_audio(filepath, target_sr=16000):
    """Load an audio file for headless/cloud execution."""
    import librosa
    print(f"\nLoading audio file: {filepath}")
    try:
        # librosa handles resampling to target_sr and mono conversion automatically
        waveform, sr = librosa.load(filepath, sr=target_sr, mono=True)
    except ImportError:
        print("  [!] 'librosa' not found. Falling back to 'soundfile' (no resampling).")
        import soundfile as sf
        waveform, sr = sf.read(filepath)
        if len(waveform.shape) > 1:
            waveform = waveform.mean(axis=1)  # to mono
        if sr != target_sr:
            print(f"  [!] Warning: File sample rate is {sr}Hz, but models expect {target_sr}Hz.")
            print("      Install librosa for automatic resampling: pip install librosa")
            
    waveform = waveform.astype(np.float32)
    duration = len(waveform) / target_sr
    print(f"  Loaded {duration:.1f}s of audio from file.\n")
...
def main():
    parser = argparse.ArgumentParser(description="Fast parallel audio analysis (File/Headless Mode)")
    parser.add_argument("--file", "-f", type=str, required=True,
                        help="Path to an audio file (.wav) to analyze (required for headless Linux/Cloud servers)")
    ...
    if not os.path.exists(args.file):
        print(f"\n[!] Error: Audio file '{args.file}' not found.\n")
        return
    audio_np, sr = load_audio(args.file)
